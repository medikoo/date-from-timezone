...
var resolveDate = function (refDate, resultDate, tokenize) {
	...
	const resolvedDate = new Date(
		tokens.year, tokens.month, tokens.date, tokens.hours, tokens.minutes,
		tokens.seconds, tokens.milliseconds
	);

	if (resolvedDate.getTime() === refDate.getTime()) return resultDate;
	resultDate.setTime(
		resultDate.getTime() + refDate.getTime() - resolvedDate.getTime()
	);
...
module.exports = function (timezone) {
	...
	return function (
		...
		const date = new Date(
			year, month, arguments[2] || 1, arguments[3] || 0, arguments[4] || 0,
